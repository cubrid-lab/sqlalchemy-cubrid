...
### Added
- **Property-based fuzz testing with Hypothesis (stabilization Phase 1)** — added `test/test_fuzz_select.py`, which generates SELECT combinations (columns, WHERE predicates, boolean connectives, LIMIT/OFFSET, ORDER/GROUP BY, subqueries, aliases, aggregates) with boundary values (`0`, `±1`, `2^31`, `2^63-1`, empty/Unicode/reserved-word strings) and asserts dialect invariants: compilation raises only `CompileError` (never an unexpected exception) and the positional-placeholder count equals the bound-parameter count. Live-execution fuzz tests (`integration`-marked) assert LIMIT/OFFSET cardinality and that compilable predicates execute on real CUBRID. Hypothesis profiles (`dev`/`ci`/`nightly`) are registered in `conftest.py`; PR CI runs the fast profile, and the nightly `integration-full` workflow runs the 2000-examples profile against live CUBRID.
