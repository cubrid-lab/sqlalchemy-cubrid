...
### Fixed
- **Reflection error translation no longer treats native -493 syntax errors as NoSuchTableError (#454)** - CUBRID native errno -493 is ER_PT_SYNTAX (general parser syntax error), not a dedicated table-not-found code. The reflection error boundary in `get_columns()` and `get_indexes()` previously treated native -493 alone as proof that a table was absent, converting syntax `ProgrammingError` failures (SQLSTATE 42000) into `NoSuchTableError`. `_is_no_such_table_error()` now requires a verified missing-object signal: either SQLSTATE 42S02 (ANSI base table or view not found) or an explicit missing-table message ('Unknown class' or 'Table not found') across pycubrid and CUBRIDdb. General syntax errors and unrelated reflection query failures now propagate their original exceptions instead of being masked as missing tables.
