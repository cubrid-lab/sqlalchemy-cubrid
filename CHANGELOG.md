...
- **BREAKING: bare `cubrid://` URL now binds the pure-Python `pycubrid` driver (#276)** — completing the 2.0 default flip announced by the 1.x deprecation warning. The bare `cubrid` entry point now resolves to `PyCubridDialect` instead of the legacy `CUBRIDdb` C-extension dialect, and the `[cubrid]` install extra now installs `pycubrid>=1.3.2,<2.0` (previously `CUBRID-Python`). The `DeprecationWarning` emitted on bare-URL engine creation in 1.x is gone. **Migration:** to keep the legacy CUBRIDdb C-extension driver, use the explicit `cubrid+cubriddb://` URL together with the `[cubriddb]` install extra (`pip install "sqlalchemy-cubrid[cubriddb]"`). The explicit `cubrid+cubrid://`, `cubrid+cubriddb://`, and `cubrid+pycubrid://` URLs are unchanged.

### Deprecated
- **Bare `cubrid://` URL now warns that its default driver will change to pycubrid in 2.0 (#276)** — the bare `cubrid://` URL currently binds the legacy `CUBRIDdb` C-extension driver. In **sqlalchemy-cubrid 2.0** it will default to the pure-Python `pycubrid` driver instead. To make this transition safe and non-breaking, the bare `cubrid` entry point is now backed by a `_DeprecatedDefaultCubridDialect` subclass that emits a `DeprecationWarning` on engine creation, and a new explicit `cubrid+cubriddb://` URL (plus a matching `cubriddb = ["CUBRID-Python"]` install extra) is available to pin the legacy driver permanently. Migrate now by choosing the driver explicitly: use `cubrid+cubriddb://` to keep CUBRIDdb, or `cubrid+pycubrid://` to adopt the pure-Python driver. The explicit `cubrid+cubrid://`, `cubrid+cubriddb://`, and `cubrid+pycubrid://` URLs do not warn. No behavior changes in 1.7 — only the warning and the new opt-in names are added; the actual default flip and the `[cubrid]` extra change will land in 2.0.

### Removed
- **BREAKING: removed the `_DeprecatedDefaultCubridDialect` transition shim (#276)** — the 1.x bare-`cubrid://` dialect subclass that emitted the `DeprecationWarning` on engine creation has been removed now that the default flip to `pycubrid` is complete (see **Changed**). The bare `cubrid` entry point resolves directly to `PyCubridDialect`.
