...
### Fixed
- **Multi-row `ON DUPLICATE KEY UPDATE` no longer emits a confusing internal error (#371)** — a multi-row INSERT (`.values([{...}, {...}])`) combined with `on_duplicate_key_update(col=stmt.inserted.col)` previously failed with an internal `cannot resolve INSERT bind parameter` error. Because CUBRID has no `VALUES(column)` equivalent, per-row inserted values cannot be referenced in a single multi-row upsert; the dialect now raises a clear, actionable `CompileError` pointing to the supported workarounds (single-row INSERTs, DBAPI `executemany`, or a static/literal UPDATE value). Multi-row ODKU with a static/literal UPDATE value now compiles and runs correctly (verified live on CUBRID 11.4).
