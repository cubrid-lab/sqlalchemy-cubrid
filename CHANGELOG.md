...
### Fixed
- **Offset-only queries no longer cap out at ~1.07e9 rows (#414)** — CUBRID rejects a bare `OFFSET` (`Syntax error: unexpected 'OFFSET'`), so an offset-only query still has to name a row count. That count was the constant `1073741823` (2**30-1), a finite ceiling rather than an "unlimited" marker, so `.offset(n)` silently truncated past ~1.07e9 rows with no error; the docs called it "max int", which it is not (the signed 32-bit maximum is 2**31-1). Raising it to a larger constant is not enough on its own: CUBRID evaluates `LIMIT offset, count` as `offset + count`, and a **prepared statement** whose sum exceeds the signed BIGINT maximum fails at execution with `-458 Overflow occurred in addition context` — invisible when the offset is a literal, because that form is folded at parse time. The row count is now rendered as `(9223372036854775807 - offset)`, keeping the sum at exactly the maximum for every offset, so the clause can neither overflow nor truncate early. Verified on live CUBRID 11.4.6 through `PREPARE`/`EXECUTE`.
