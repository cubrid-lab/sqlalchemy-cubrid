...
### Fixed
- **Offset-only queries no longer cap out at ~1.07e9 rows (#414)** — CUBRID rejects a bare `OFFSET` (`Syntax error: unexpected 'OFFSET'`), so every paginating form requires a row count and an offset-only query has to invent one; this dialect renders `LIMIT offset, <row count>`. The row-count sentinel was `1073741823` (2**30-1), a finite cap rather than an "unlimited" marker, so `.offset(n)` silently truncated past that many rows with no error. The docs called it "max int", which it is not — the signed 32-bit maximum is 2**31-1. Raised to `9223372036854775807` (the signed BIGINT maximum), verified accepted by live CUBRID 11.4, and extracted as a named `_MAX_ROW_COUNT` constant so the test asserts on the constant instead of re-encoding the literal.
