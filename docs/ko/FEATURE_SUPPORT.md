...
- **LIMIT / OFFSET**: CUBRID는 MySQL 스타일 `LIMIT [offset,] count` 구문을 사용합니다. 오프셋만 주어지면 방언은 "남은 모든 행" 센티넬로 `LIMIT offset, 9223372036854775807`(부호 있는 BIGINT 최댓값)을 내며, 이는 SQLAlchemy의 MySQL 방언이 오프셋 단독을 처리하는 방식과 동일합니다.
