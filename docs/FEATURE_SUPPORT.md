...
- **LIMIT / OFFSET**: the dialect emits MySQL-style `LIMIT [offset,] count`. CUBRID has no bare `OFFSET`, so an offset-only query still has to name a row count. It cannot be a plain constant: CUBRID evaluates the clause as `offset + count` and a prepared statement whose sum exceeds the signed BIGINT maximum fails with `-458 Overflow occurred in addition context`. The dialect therefore renders `LIMIT offset, (9223372036854775807 - (offset))`, which holds that sum at exactly the maximum for any offset. Verified on CUBRID 11.4.6.
